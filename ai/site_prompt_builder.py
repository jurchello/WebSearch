...
class SitePromptBuilder(BasePromptBuilder):
    ...
    def get_user_message(self, data: AIDomainData) -> str:
        ...
        # Regular section
        if not data.regular_country_codes:
            ...
            regular_codes_str = ", ".join(sorted(data.regular_country_codes))

        # Community section
        if data.community_country_codes:
            community_codes_str = ", ".join(sorted(data.community_country_codes))
        else:
        ...
        return (
            ...
            "- 5 based on community-curated sources like groups, channels, "
